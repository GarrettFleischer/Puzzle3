...
//    m_sayings.append("Ronda Rousey.");
    ...
    m_question = this->get_random_question();
}

QString magic_8_ball::get_random_question()
{
    return m_questions.at(qFloor(qrand() % m_questions.length()));
}

QString magic_8_ball::get_saying()
{
    return m_sayings.at(qFloor((qrand() % m_sayings.length())));
}
